...
public class MovingSawPath : MonoBehaviour {
	...
	void Update () {
        ...
        sawObj.transform.position = Vector3.MoveTowards( sawObj.transform.position, pointToGo.position, smoothing * Time.deltaTime );

        if( sawObj.transform.position == pointToGo.position ) {
            pointIndex++;
